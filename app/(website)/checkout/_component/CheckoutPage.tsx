...
export default function CheckoutFormWrapper({ userId, serviceId, amount, type }: CheckoutFormWrapperProps) {
  const { data: clientSecret, error, isPending } = useQuery({
    ...
    enabled: !!userId && !!amount, // Only run if userId and amount exist
    refetchOnWindowFocus: false,
  });

  if (error) return <div className="text-red-600 p-4">{(error as Error).message}</div>;
  if (isPending || !clientSecret) return <div className="p-4">Loading payment information...</div>;
