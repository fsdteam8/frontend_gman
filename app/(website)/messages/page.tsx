...
export default function MessagesPage() {
        ...
        setChats(prevChats => {
          ...
          if (chatIndex >= 0) {
            ...
            const updatedChat = {
              ...
              messages: [message], // Update with new message
              updatedAt: new Date().toISOString(),
            };

            // If this is not the selected chat, increment unread count
  ...
  // Join chat room when selecting a chat and mark messages as read
  useEffect(() => {
    if (selectedChat && socketRef.current) {
      socketRef.current.emit("joinChatRoom", selectedChat._id);
      ...
      // Update chats to reset unread count for this chat
      setChats(prevChats => 
        prevChats.map(chat => 
  ...
  const markMessagesAsRead = async (chatId: string) => {
    ...
    try {
      await fetch(`${BASE_URL}/chat/mark-as-read/${chatId}`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const fetchChats = async () => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        // Sort chats by updatedAt (newest first) and add unreadCount
        const sortedChats = data.data.map((chat: Chat) => ({
          ...chat,
          unreadCount: chat.messages.reduce((count, msg) => 
            !msg.read && (typeof msg.user === 'string' ? msg.user !== currentUser?._id : msg.user._id !== currentUser?._id) 
              ? count + 1 
              : count, 
            0
          ),
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setChats(prevChats => {
          const chatIndex = prevChats.findIndex(chat => chat._id === selectedChat._id);
          if (chatIndex >= 0) {
            const updatedChats = [...prevChats];
            const updatedChat = {
              ...updatedChats[chatIndex],
